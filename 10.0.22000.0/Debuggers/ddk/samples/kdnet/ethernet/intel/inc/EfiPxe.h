...
//
// N.B. This struct is non-standard. The last 8 bytes (LinkSpeed, LinkDuplex)
//      are KDNET specific. Ensure that KDNET extensibility modules which use
//      their own UNDI implementation take this into account. For example,
//      broadcom uses PXE_DB_INITIALIZE_CU in place of their regular
//      PXE_DB_INITIALIZE struct when compiling their UNDI for use with KDNET.
//
typedef struct s_pxe_db_initialize {
  ...
  //
  // Actual link speed and duplex after initialization.
  //
  UINT32 LinkSpeed;
  UINT32 LinkDuplex;
} PXE_DB_INITIALIZE;
