    ...
    _In_ PUSBFNMP_CONTEXT Context)
{
    ...
    if (DCTL.RunStop == 1)
    {
        LOG_TRACE("Controller is running");
        DCTL.RunStop = 0;
    ...
    //
    DCTL.AsUlong32 = INREG32(&UsbCtrl->DeviceRegisters.Common.Control.AsUlong32);
    LOG_TRACE("READ DCTL: 0x%08x 0x%08x", &UsbCtrl->DeviceRegisters.Common.Control.AsUlong32, DCTL.AsUlong32);
    DCTL.CoreSoftReset = 1;
    OUTREG32(&UsbCtrl->DeviceRegisters.Common.Control.AsUlong32, DCTL.AsUlong32);
    LOG_TRACE("WRITE DCTL: 0x%08x 0x%08x", &UsbCtrl->DeviceRegisters.Common.Control.AsUlong32, DCTL.AsUlong32);
    ...
    LOG_TRACE("READ GCTL: 0x%08x 0x%08x", &UsbCtrl->GlobalRegisters.Common.CoreControl.AsUlong32, GCTL.AsUlong32);
    GCTL.PortCapabilityDirection = PORT_CAPABILITY_DEVICE_CONFIGURATION;

    //
    // Improve SS enumeration by setting the retry SS bit.
    // In general, If the SS connection fails during Poll/LMP exchange, the device
    // will try to connect in non-SS mode, but If this bit is set, then device
    // attempts three more times to connect at SS.
    // This bit also helps on SS re-connecting for plugging in/out USB cable.
    //

    GCTL.RetrySSConnection = 1;
    OUTREG32(&UsbCtrl->GlobalRegisters.Common.CoreControl.AsUlong32, GCTL.AsUlong32);
    LOG_TRACE("WRITE GCTL: 0x%08x 0x%08x", &UsbCtrl->GlobalRegisters.Common.CoreControl.AsUlong32, GCTL.AsUlong32);
    ...
    LOG_TRACE("WRITE Event-Hi: 0x%08x 0x%08x", &UsbCtrl->GlobalRegisters.EventBuffers[0].EventBufferAddress.Hi, Context->DmaPa.HighPart);
    OUTREG32(&UsbCtrl->GlobalRegisters.EventBuffers[0].EventBufferAddress.Lo,
             Context->DmaPa.LowPart);
    LOG_TRACE("WRITE Event-Lo: 0x%08x 0x%08x", &UsbCtrl->GlobalRegisters.EventBuffers[0].EventBufferAddress.Lo, Context->DmaPa.LowPart);
    ...
    LOG_TRACE("READ Event-Size: 0x%08x 0x%08x", &UsbCtrl->GlobalRegisters.EventBuffers[0].EventSize.AsUlong32, GEVTSIZ0.AsUlong32);
    ...
    LOG_TRACE("WRITE Event-Size: 0x%08x 0x%08x", &UsbCtrl->GlobalRegisters.EventBuffers[0].EventSize.AsUlong32, GEVTSIZ0.AsUlong32);
    ...
    LOG_TRACE("READ Event-Count: 0x%08x 0x%08x", &UsbCtrl->GlobalRegisters.EventBuffers[0].EventCount.AsUlong32, EVNTCOUNT0.AsUlong32);
    EVNTCOUNT0.Count = 0;
    OUTREG32(&UsbCtrl->GlobalRegisters.EventBuffers[0].EventCount.AsUlong32, EVNTCOUNT0.AsUlong32);
    LOG_TRACE("WRITE Event-Count: 0x%08x 0x%08x", &UsbCtrl->GlobalRegisters.EventBuffers[0].EventCount.AsUlong32, EVNTCOUNT0.AsUlong32);
    ...
    LOG_TRACE("READ DCFG: 0x%08x 0x%08x", &UsbCtrl->DeviceRegisters.Common.Configuration.AsUlong32, DCFG.AsUlong32);
    DCFG.MaxDeviceSpeed = DEVICE_SPEED_SUPER_SPEED;

    //
    // Disable LPM. Some versions of the Synopsys core have issues with
    // handling LPM requests from the host USB driver. See http://osgvsowi/11035552.
    // The Windows function driver also has this bit disable in the Soft Reset sequence.
    //

    DCFG.LPMCapable = 0;
    OUTREG32(&UsbCtrl->DeviceRegisters.Common.Configuration.AsUlong32, DCFG.AsUlong32);
    LOG_TRACE("WRITE DCFG: 0x%08x 0x%08x", &UsbCtrl->DeviceRegisters.Common.Configuration.AsUlong32, DCFG.AsUlong32);
    ...
    LOG_TRACE("READ Dev-Event: 0x%08x 0x%08x", &UsbCtrl->DeviceRegisters.Common.EnabledEvents.AsUlong32, DEVTEN.AsUlong32);
    DEVTEN.USBReset = 1;
    ...
    LOG_TRACE("WRITE Dev-Event: 0x%08x 0x%08x", &UsbCtrl->DeviceRegisters.Common.EnabledEvents.AsUlong32, DEVTEN.AsUlong32);
