...
# error Unsupported version of Visual Studio. Verify.h requires VS 2015 or later.
#endif
...
// Make it so #if __has_include(<foo>) defaults to false while the compiler does not support this construct.
// VS 2017 supports it but VS 2015 does not.
...
#  if !defined(TAEF_NO_SYSTEM_ERROR_SUPPORT) && __has_include(<system_error>)
...
#   define VERIFY_THROWS_SPECIFIC_CX(__operation, __exception, __func, ...)                                                                          \
{                                                                                                                                                    \
    bool __exceptionHit = false;                                                                                                                     \
                                                                                                                                                     \
    try                                                                                                                                              \
...
#   define VERIFY_THROWS_CX(__operation, __exception, ...)                                                                                           \
        VERIFY_THROWS_SPECIFIC_CX(__operation, __exception, WEX::TestExecution::Private::TrueUnaryFunctor<__exception^>(), __VA_ARGS__)
...
{
    // Users can implement VerifyOutputTraits specializations for their classes to provide customed log output.
    template <typename T>
...
#define BUILD_OUTPUT_TRAIT(Type, FormatString) \
    ...
    namespace Private
    {
        extern "C"
        {
            ...
            WEXLOGGER_API int WEXLOGGER_STDCALL GetDisabledVerifyThrowCount();
            WEXLOGGER_API void WEXLOGGER_STDCALL IncrementDisabledVerifyThrowCount();
            WEXLOGGER_API void WEXLOGGER_STDCALL DecrementDisabledVerifyThrowCount();
        }
    }
    ...
    namespace Private
    {
        extern "C"
        {
            ...
            WEXLOGGER_API VerifyOutputSettings::Setting WEXLOGGER_STDCALL GetVerifyOutputSettings();
            WEXLOGGER_API void WEXLOGGER_STDCALL SetVerifyOutputSettings(VerifyOutputSettings::Setting settings);
        }
    }
    ...
    namespace Private
    {
        ...
        public:
            ...
            static bool ExpectedExceptionThrown(const T& TException, const wchar_t* pszException, const wchar_t* pszOperation, const wchar_t* pszMessage = nullptr)
            {
                ...
                return WEX::TestExecution::Verify::VerificationPassed([&]()->WEX::Common::NoThrowString{ return BuildMessage(c_szFormat, pszException, pszOperation, pszMessage); });
            }
            ...
            static bool ExpectedExceptionNotThrown(const wchar_t* pszException, const wchar_t* pszOperation, const ErrorInfo& errorInfo, const wchar_t* pszMessage = nullptr)
            {
                ...
                return WEX::TestExecution::Verify::VerificationFailed([&]()->WEX::Common::NoThrowString { return BuildMessage(c_szFormat, pszException, pszOperation, pszMessage); },
                    errorInfo);
            }
            ...
            static bool UnexpectedExceptionThrown(const wchar_t* pszOperation, const ErrorInfo& errorInfo, const wchar_t* pszMessage = nullptr)
            {
                ...
                return WEX::TestExecution::Verify::VerificationFailed([&]()->WEX::Common::NoThrowString { return BuildMessage(c_szFormat, pszOperation, pszMessage); },
                    errorInfo);
            }
            ...
            static bool UnexpectedExceptionThrownWithSpecifics(const wchar_t* pszOperation, const ErrorInfo& errorInfo, const wchar_t* pszSpecificException, const wchar_t* pszMessage = nullptr)
            {
                ...
                return WEX::TestExecution::Verify::VerificationFailed([&]()->WEX::Common::NoThrowString { return BuildMessage(c_szFormat, pszSpecificException, pszOperation, pszMessage); },
                    errorInfo);
            }
            ...
            static bool UnexpectedExceptionNotThrown(const wchar_t* pszOperation, const wchar_t* pszMessage = nullptr)
            {
                ...
                return WEX::TestExecution::Verify::VerificationPassed([&]()->WEX::Common::NoThrowString { return BuildMessage(c_szFormat, pszOperation, pszMessage); });
            }
...
# if defined(_WIN32)
            if (errorCode.category() == std::system_category())
            {
                ...
                // An errorCode of 0 is not expected for an exception. Fall back to E_UNEXPECTED.
            }
# endif
...
# if defined(TAEF_CPPWINRT_SUPPORT)
            catch (const winrt::hresult_error& e)
            {
                ...
                static const wchar_t c_szMessage[] = L"Caught winrt::hresult_error";
                message.Format(L"%s: %s [HRESULT: 0x%x]", c_szMessage, e.message().c_str(), e.code().value);
                return message.ToCStrWithFallbackTo(c_szMessage);
            }
...
# if defined(TAEF_SYSTEM_ERROR_SUPPORT)
            catch (const std::system_error& e)
            {
                ...
                const HRESULT errorCode = GetHResultForStdErrorCode(e.code(), isExactConvesion);
                if (pHResult)
                {
                    *pHResult = errorCode;
                }

                static const wchar_t c_szMessage[] = L"Caught std::system_error";
                ...
                message.TrimRight();
                // If we have an exact HRESULT value to use, also append that to the message.
                if (isExactConvesion)
                {
                    message.AppendFormat(L" [HRESULT: 0x%x]", errorCode);
                }
                return message.ToCStrWithFallbackTo(c_szMessage);
            }
# endif // if defined(TAEF_SYSTEM_ERROR_SUPPORT)
