...
namespace GdbSrvControllerLib
{
    ...
    typedef struct 
    {
        WORD isReadPhysical: 1;    //  Set if the query is to read physical memory
        WORD isSupervisor: 1;      //  Set if the query is to read supervisor/system mode memory 
        WORD isSpecialRegs: 1;     //  Set if the query is to access memory pointed by Special registers
        WORD isData:1;             //  Set if the query is to read User mode memory.
        WORD isHypervisor:1;       //  Set if the query is to read Hypervisor memory.
        WORD fUnUsed: 11;
    } memoryReadType;
    ...
    public:
        ...
        SimpleCharBuffer ReadMemory(_In_ AddressType address, _In_ size_t size, _In_ memoryReadType memType);
        ...
        bool WriteMemory(_In_ AddressType address, _In_ size_t size, _In_ const void * pRawBuffer, 
                         _Out_ DWORD * pdwBytesWritten, _In_ bool isPhysicalMemory = false);
        ...
        void GetMemoryPacketType(_In_ DWORD64 cpsrRegValue, _Out_ memoryReadType * pMemType);
