...
class GdbSrvController::GdbSrvControllerImpl
{
    ...
    SimpleCharBuffer GdbSrvControllerImpl::ReadMemory(_In_ AddressType address, _In_ size_t maxSize, 
                                                      _In_ const memoryReadType memType)
    {
    ...
    bool GdbSrvControllerImpl::WriteMemory(_In_ AddressType address, _In_ size_t size, _In_ const void * pRawBuffer, 
                                           _Out_ DWORD * pdwBytesWritten, _In_ bool isPhysicalMemory)
    {
        ...
        for (;;)
        {
            ...
            PCSTR pFormat = GetWriteMemoryCmd(isPhysicalMemory);
    ...
    void GdbSrvControllerImpl::GetMemoryPacketType(_In_ DWORD64 cpsrRegValue, _Out_ memoryReadType * pMemType)
    {
    ...
    PCSTR GetReadMemoryCmd(_In_ memoryReadType memType)
    {
        PCSTR pFormat = nullptr;
        
        if (m_pRspClient->IsFeatureEnabled(PACKET_READ_TRACE32_SPECIAL_MEM) && memType.isReadPhysical)
        {
    ...
    PCSTR GetWriteMemoryCmd(_In_ bool isPhysicalMemory)
    {
        PCSTR pFormat;
        
        if (isPhysicalMemory && m_pRspClient->IsFeatureEnabled(PACKET_READ_TRACE32_SPECIAL_MEM))
        {
             pFormat = (m_targetProcessorArch == ARM64_ARCH || m_targetProcessorArch == AMD64_ARCH) ?
                       "Qtrace32.memory:a,%I64x," : "Qtrace32.memory:a,%x,";
        }
        else
        {
             pFormat = (m_targetProcessorArch == ARM64_ARCH || m_targetProcessorArch == AMD64_ARCH) ?
...
SimpleCharBuffer GdbSrvController::ReadMemory(_In_ AddressType address, _In_ size_t size, _In_ const memoryReadType memType)
{
    assert(m_pGdbSrvControllerImpl != nullptr);
                                   ...
                                   _Out_ DWORD * pdwBytesWritten, _In_ bool isPhysicalMemory)
{
    ...
    return m_pGdbSrvControllerImpl->WriteMemory(address, size, pRawBuffer, pdwBytesWritten, isPhysicalMemory);
}
...
void GdbSrvController::GetMemoryPacketType(_In_ DWORD64 cpsrRegValue, _Out_ memoryReadType * pMemType)
{
    assert(m_pGdbSrvControllerImpl != nullptr);    
