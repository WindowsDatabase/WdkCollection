...
Abstract:
    ...
    Qualcomm SDM 845 serial support.
...

{
   ...
   int temp;
   UART_DM_WRITE_REG(DebugPort->Port.Address + GENI4_CFG, HWIO_GENI_DFS_IF_CFG_OFFS, 0x0);

   temp = ( HWIO_DMA_GENERAL_CFG_AHB_SEC_SLV_CLK_CGC_ON_BMSK  << HWIO_DMA_GENERAL_CFG_AHB_SEC_SLV_CLK_CGC_ON_SHFT) |
   ...
   UART_DM_WRITE_REG(DebugPort->Port.Address + QUPV3_SE_DMA, HWIO_DMA_GENERAL_CFG_OFFS, temp);

   UART_DM_WRITE_REG(DebugPort->Port.Address + GENI4_CFG, HWIO_GENI_CGC_CTRL_OFFS, 0x7F);

   UART_DM_WRITE_REG(DebugPort->Port.Address + GENI4_CFG, HWIO_GENI_FORCE_DEFAULT_REG_OFFS, 0x1);

   UART_DM_WRITE_REG(DebugPort->Port.Address + GENI4_CFG, HWIO_GENI_OUTPUT_CTRL_OFFS, 0x7F);

   UART_DM_WRITE_REG(DebugPort->Port.Address + GENI4_IMAGE_REGS, HWIO_GENI_DMA_MODE_EN_OFFS, 0);

   UART_DM_WRITE_REG(DebugPort->Port.Address + QUPV3_SE_DMA, HWIO_SE_IRQ_EN_OFFS, 0xFFFFFFFF);

   UART_DM_WRITE_REG(DebugPort->Port.Address + QUPV3_SE_DMA, HWIO_SE_GSI_EVENT_EN_OFFS, 0);

   UART_DM_WRITE_REG(DebugPort->Port.Address + GENI4_DATA, HWIO_GENI_M_IRQ_ENABLE_OFFS, 0xB300005F);
   
   UART_DM_WRITE_REG(DebugPort->Port.Address + GENI4_DATA, HWIO_GENI_S_IRQ_ENABLE_OFFS, 0xB3007E5F);

   temp = UART_DM_READ_REG(DebugPort->Port.Address + QUPV3_SE_DMA, HWIO_SE_HW_PARAM_0_OFFS); // to get TXFIFO width

   temp = (temp & TX_FIFO_DEPTH_MASK) >> TX_FIFO_DEPTH_SHIFT;

   UART_DM_WRITE_REG(DebugPort->Port.Address + GENI4_DATA, HWIO_GENI_TX_WATERMARK_REG_OFFS, 4);

   temp = UART_DM_READ_REG(DebugPort->Port.Address + QUPV3_SE_DMA, HWIO_SE_HW_PARAM_1_OFFS);// to get RXFIFO width

   temp = (temp & RX_FIFO_DEPTH_MASK) >> RX_FIFO_DEPTH_SHIFT;

   UART_DM_WRITE_REG(DebugPort->Port.Address + GENI4_DATA, HWIO_GENI_RX_WATERMARK_REG_OFFS, temp - 8);

   UART_DM_WRITE_REG(DebugPort->Port.Address + GENI4_DATA, HWIO_GENI_RX_RFR_WATERMARK_REG_OFFS, temp - 4);

   SDM845SetBaud(&DebugPort->Port, DebugPort->Port.Baud);


   UART_DM_WRITE_REG(DebugPort->Port.Address + GENI4_IMAGE_REGS, HWIO_UART_TX_WORD_LEN_OFFS, 0x8);

   UART_DM_WRITE_REG(DebugPort->Port.Address + GENI4_IMAGE_REGS, HWIO_UART_RX_WORD_LEN_OFFS, 0x8);

   UART_DM_WRITE_REG(DebugPort->Port.Address + GENI4_IMAGE_REGS, HWIO_UART_TX_PARITY_CFG_OFFS, 0x0);

   UART_DM_WRITE_REG(DebugPort->Port.Address + GENI4_IMAGE_REGS, HWIO_UART_TX_TRANS_CFG_OFFS, 0x2);

   UART_DM_WRITE_REG(DebugPort->Port.Address + GENI4_IMAGE_REGS, HWIO_UART_RX_PARITY_CFG_OFFS, 0x0);

   UART_DM_WRITE_REG(DebugPort->Port.Address + GENI4_IMAGE_REGS, HWIO_UART_RX_TRANS_CFG_OFFS, 0x0);

   UART_DM_WRITE_REG(DebugPort->Port.Address + GENI4_IMAGE_REGS, HWIO_UART_TX_STOP_BIT_LEN_OFFS, 0x0);

   UART_DM_WRITE_REG(DebugPort->Port.Address + GENI4_IMAGE_REGS, HWIO_UART_RX_STALE_CNT_OFFS, 0x16*10);

   UART_DM_WRITE_REG(DebugPort->Port.Address+ GENI4_DATA, HWIO_GENI_S_CMD0_OFFS, 0x8000000);
...
--*/
{
   ...
   temp = (DivisorLatch << (HWIO_GENI_SER_M_CLK_CFG_CLK_DIV_VALUE_SHFT)) |
   HWIO_GENI_SER_M_CLK_CFG_SER_CLK_EN_BMSK;

   UART_DM_WRITE_REG(pPort->Address + GENI4_CFG,
   HWIO_GENI_SER_M_CLK_CFG_OFFS, temp);

   UART_DM_WRITE_REG(pPort->Address + GENI4_CFG,
   HWIO_GENI_SER_S_CLK_CFG_OFFS, temp);
}
...

{
    ...
    if (tx_started != 0) {
        while(!(UART_DM_READ_REG(pPort->Address + GENI4_DATA, HWIO_GENI_M_IRQ_STATUS_OFFS) & 0x1));
        UART_DM_WRITE_REG(pPort->Address + GENI4_DATA, HWIO_GENI_M_IRQ_CLEAR_OFFS, 0x1);
    }

    UART_DM_WRITE_REG(pPort->Address + GENI4_IMAGE_REGS, HWIO_UART_TX_TRANS_LEN_OFFS, 1);
    UART_DM_WRITE_REG(pPort->Address + GENI4_DATA, HWIO_GENI_M_CMD0_OFFS, 0x8000000);
    tx_started = 1;

    UINT32 word_value = 0;
    word_value = word_value | Byte;
    UART_DM_WRITE_REG(pPort->Address + GENI4_DATA, HWIO_GENI_TX_FIFOn_OFFS(pPort->Address, 0), word_value);
...

{
    ...
    UINT32  i = 0;
    UINT32 array_loc = 0;
    static UINT32  avail_bytes = 0;
    static PUCHAR byte_buf = NULL;
    ...
    if (*CompDbgPortsPresent == FALSE) {
            SDM845SetBaud(pPort, pPort->Baud);
	...
	{
		byte_buf = (UCHAR*) buf;
		s_irq_status = UART_DM_READ_REG (Address + GENI4_DATA, HWIO_GENI_S_IRQ_STATUS_OFFS);

		UART_DM_WRITE_REG(Address + GENI4_DATA, HWIO_GENI_S_IRQ_CLEAR_OFFS, s_irq_status);
		rx_fifo_status = UART_DM_READ_REG(Address + GENI4_DATA, HWIO_GENI_RX_FIFO_STATUS_OFFS);
		...
		if (partial_bytes_to_read > 0 || words_to_read > 0)
		{
		...
		{
			words_to_read = UART_DM_READ_REG(Address + GENI4_DATA, HWIO_GENI_RX_WATERMARK_REG_OFFS);
		}

		avail_bytes = (words_to_read * 4) + partial_bytes_to_read;

		if ( (buf - (UCHAR *)NULL) & 0x03 )  // buffer is not 32-bit word aligned
		{
			for (i = 0; i < words_to_read;)
			{
				rx_word = UART_DM_READ_REG(Address + GENI4_DATA, HWIO_GENI_RX_FIFOn_OFFS(Address, i));
				buf[0 + array_loc] = (UCHAR)(rx_word >>  0);
				buf[1 + array_loc] = (UCHAR)(rx_word >>  8);
				buf[2 + array_loc] = (UCHAR)(rx_word >> 16);
				buf[3 + array_loc] = (UCHAR)(rx_word >> 24);
				array_loc  += 4;
			 }
		}
		...
		{
				rx_word = UART_DM_READ_REG(Address + GENI4_DATA, HWIO_GENI_RX_FIFOn_OFFS(Address, i));
			for (i = 0; i < partial_bytes_to_read; i++) {
				buf[i] = (UCHAR)(rx_word >> i * 8); 
			}
...
KD_HARDWARE_LAYER_DRIVER SDM845HardwareLayerDriver = {
    SDM845InitializePort,
    SDM845PutByte,
    SDM845GetByte,
