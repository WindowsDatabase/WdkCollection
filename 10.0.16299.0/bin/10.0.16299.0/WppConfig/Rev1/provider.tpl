...
#define `Provider.ProviderVarName`Handle (`Provider.ProviderSymbol`_Context.RegistrationHandle)
...
EtwppTemplate_`i.Name`(
    __in MCGEN_TRACE_CONTEXT* Context,
...
#define ARGUMENT_COUNT_`i.Name` `i.Count`
    ...
    ULONG Result = EventWrite(Context->RegistrationHandle, Descriptor, ARGUMENT_COUNT_`i.Name`, (ARGUMENT_COUNT_`i.Name` == 0) ? NULL : EventData);
    return Result;
}
...
#define EventWrite_`Msg.Name`(`Msg.FixedArgs``Msg.MacroArgs`)\
        ...
        Etwpp`Msg.EtwTemplateSymbol`(&`Provider.ProviderSymbol`_Context, &`Msg.Func`_`Msg.EtwId``Msg.MacroExprs`)\
        : ERROR_SUCCESS\
