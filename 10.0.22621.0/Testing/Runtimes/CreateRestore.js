...
// Make sure that the system restore service is enabled
try {
    var wmiService = GetObject("winmgmts:{impersonationLevel=impersonate}!\\\\.\\root\\default");
    var systemRestoreItem = wmiService.Get("SystemRestore");
    result = systemRestoreItem.Enable("");
} catch (e) {
    result = e.number;
    logFile.WriteLine("An exception occurred while enabling the system restore service: " + e.description);
}

if (0 == result) {
    logFile.WriteLine("System restore service is enabled.");
}
else {
    logFile.WriteLine("Failed to enable the system restore service.  Error Code: " + result.toString());
    logFile.Close();
    WScript.Quit(result);
}

// Enumerate previous restore points, if any
