...
//----------------------------------------------------------------------------------------------------------------------
#pragma once
// The test data retrieval APIs are not yet implemented on Linux.
#if defined(_WIN32)
#include "TE.Common.h"
...
#if defined(__GNUC__) /*clang and gcc on linux*/
...
#elif defined(__clang__) /*clang on Windows*/
...
# pragma clang diagnostic pop
    ...
    public:
        ...
        static HRESULT TAEF_STDCALL TryGetValue(_In_z_ const wchar_t* pszString, T& result)
        {
            static_assert(Private::AreSameType<int, T>::value || Private::AreSameType<unsigned int, T>::value
                || Private::AreSameType<size_t, T>::value || Private::AreSameType<bool, T>::value
                || Private::AreSameType<float, T>::value || Private::AreSameType<double, T>::value
                || Private::AreSameType<long, T>::value || Private::AreSameType<unsigned long, T>::value
#if defined(_WIN32)
