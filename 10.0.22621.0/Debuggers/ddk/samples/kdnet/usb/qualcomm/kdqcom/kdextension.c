    ...
    )
{
    ...
    //
    // Ensure the vendor ID is not selecting the MSHV_NET transport.
    // This check MUST NOT be removed, so that shared boot and hyper-v
    // debugging work correctly.
    //

    if (KdNet->Device->VendorID == PCI_VID_MSHV_NET) {
