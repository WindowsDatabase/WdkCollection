...
--*/
{
  ...
  if (ActiveInterfaces == 0) {
    DEBUGPRINT(DECODE, ("No active interfaces\n"));
    CdbPtr->StatFlags = PXE_STATFLAGS_COMMAND_FAILED;
    CdbPtr->StatCode  = PXE_STATCODE_INVALID_CDB;
    return ;
  }

  if (e1000_UNDI32DeviceList[CdbPtr->IFnum] == NULL) {
    DEBUGPRINT(DECODE, ("Device entry invalid for specified IFnum %d\n", CdbPtr->IFnum));
    CdbPtr->StatFlags = PXE_STATFLAGS_COMMAND_FAILED;
    CdbPtr->StatCode  = PXE_STATCODE_INVALID_CDB;
    return ;
  }
