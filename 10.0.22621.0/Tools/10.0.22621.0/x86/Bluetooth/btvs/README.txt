...
# Command Line Options
    ...
    btvs.exe [-Mode Frontline|Ellisys|Wireshark] [-Address 127.0.0.1] [-Port 24352] [-Service 1|2|3] [-Remote off|on]
        ...
        Mode        Optionally specify whether btvs.exe should generate traces
                        for Frontline, Ellisys, or Wireshark. If mode is not specified,
                        then btvs will try to launch Frontline. If Frontline is unavailable,
                        then btvs will try to launch Wireshark.
...
2. If Frontline is installed, then Frontline will automatically open.
3. Click the `Start Capture` button (Red button on the tool bar).
4. Click `View->Frame Display` to show HCI traces.
