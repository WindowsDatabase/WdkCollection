...
#pragma detect_mismatch("ODR_violation_bthddi_mismatch", "1")
#if (_MSC_VER > 1020)
#pragma once
#endif // (_MSC_VER > 1020)
...
//! - STATUS_PROTOCOL_UNREACHBLE: remote device did not accept the l2cap connection with the specified PSM.
//! - STATUS_DEVICE_CONFIGURATION_ERROR: remote device did not agree to L2cap default configuration parameters and/or MTU size.
...
typedef struct _BRB_HEADER
{
    ...
    //! [PRIVATE] BRB Version:
    //!  - BLUETOOTH_V1: Bluetooth 1.x version.
    //!  - BLUETOOTH_V2: Bluetooth 2.x version or later, filled in by BthCreateBrb.
...
typedef struct _L2CAP_FLOWSPEC
{
    ...
    //! Token rate in bytes/sec.
    ULONG TokenRate;

    //! Token bucket size in bytes.
    ULONG TokenBucketSize;

    //! Peak badwidth in bytes/sec.
    ULONG PeakBandwidth;

    //! Latency in microsoeonds.
    ULONG Latency;
...
// For read/write transfer flags validation.
...
struct _BRB_SCO_GET_SYSTEM_INFO
{
    ...
    //! \note: Set to -1 if no limit or unknown.
    ULONG MaxChannels;

    //! [OUT] Minimum transfer in msec x request.
    //! \note: Set to -1 if variable or unknown.
    ULONG TransferUnit;
...
//! config option.
//!
//! \note: If unspecified and the remote host rejects the config request due to an extra
...
//!
//! \note: If unspecified, the extra config options will be rejected as unknown options.
...
//!
//! \note: If this flag is not specified, any reconfig is rejected and the channel is torn down.
...
typedef struct _INDICATION_PARAMETERS
{
    ...
    union
    {
        ...
        struct
        {
            ...
            //! If Response != CONFIG_STATUS_SUCCESS, then this parameter will
...
//! Caller wants to know about the device being paired
...
struct _BRB_L2CA_OPEN_ENHANCED_CHANNEL
{
    ...
    //! Object to be passed to ObReferenceObject, ObDereferenceObject.
    //! \note: If a callback is requested, this parameter is not optional.
    PVOID ReferenceObject;
...
struct _BRB_L2CA_OPEN_CHANNEL
{
    ...
    //! Combination of CALLBACK_Xxx flags.
    ULONG CallbackFlags;
...
struct _BRB_GET_DEVICE_INTERFACE_STRING
{
    ...
    //! this field contains the required number of bytes. Upon success, the
...
//!  - STATUS_INSUFFICIENT_RESOURCES: Failed to allocate memory for the request.
//!  - STATUS_INVALID_CID: Client already owns this PSM.
...
//! \returns: Brb pointer or NULL if the system is out of memory
//! \note: this function is not exported on 1.x bluetooth versions.
_IRQL_requires_same_
_Must_inspect_result_
_When_(return!=0, __drv_allocatesMem(Mem))
typedef PBRB (*PFNBTH_ALLOCATE_BRB)(_In_ BRB_TYPE brbType, _In_ ULONG tag);

//! Frees a Brb allocated by BthAllocateBrb.
//! \note: this function is not exported on 1.x bluetooth versions.
_IRQL_requires_same_
...
//! This function is use to reinitialize brb for reuse.
//! \note: this function is not exported on 1.x bluetooth versions.
_IRQL_requires_same_
typedef VOID (*PFNBTH_REUSE_BRB)(_Inout_ PBRB pBrb, _In_ BRB_TYPE brbType);

//! Indicate if the installed Bluetooth binary set supports the requested version.
//! \returns: TRUE if the installed bluetooth binaries support the given Major & Minor versions.
//! \note: this function is not exported on 1.x bluetooth versions.
_IRQL_requires_same_
_Must_inspect_result_
...
typedef struct _BTH_PROFILE_DRIVER_INTERFACE
{
    ...
    //! [OUT] Use this function to allocate Brb
    PFNBTH_ALLOCATE_BRB BthAllocateBrb;

    //! [OUT] Use this function to free Brb allocated with BthAllocateBrb
    PFNBTH_FREE_BRB BthFreeBrb;

    //! [OUT] Use this function to initialize stack allocated Brbs
    PFNBTH_INITIALIZE_BRB BthInitializeBrb;

    //! [OUT] Use this function to reinitialize Brb for reuse
    PFNBTH_REUSE_BRB BthReuseBrb;

    //! [OUT] Indicates if the installed Bluetooth binary set supports the requested version
...
{
    ENUMERATOR_ACTION_CREATE = 0,   //!< Enumerate the PDO, the device is in range.
    ENUMERATOR_ACTION_REMOVE,       //!< Remove the PDO, the device is in range.
    ENUMERATOR_ACTION_DESTROY,      //!< Force remove the PDO, the user no longer wants to use this protocol/service.
