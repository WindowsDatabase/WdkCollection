...
typedef struct NETRSSQUEUEGROUP_INIT *PNETRSSQUEUEGROUP_INIT;
...
typedef
_Function_class_(EVT_NET_ADAPTER_CREATE_RSSQUEUE_GROUP)
_IRQL_requires_same_
_IRQL_requires_max_(PASSIVE_LEVEL)
NTSTATUS
...
typedef struct _NET_ADAPTER_DMA_CAPABILITIES
{
    ...
    //
    // Maximum number of physical fragments per packet the adapter
    // can handle, or ((LONGLONG) -1) if there is no limit
    //
    SIZE_T MaximumNumberOfPhysicalFragments;
...
typedef struct _NET_ADAPTER_CONFIG {
    ...
    PFN_NET_ADAPTER_CREATE_RSSQUEUE_GROUP     EvtAdapterCreateRssQueueGroup;
    ...
    )
{
    ...
    AdapterConfig->EvtAdapterCreateRssQueueGroup = NULL;
}
