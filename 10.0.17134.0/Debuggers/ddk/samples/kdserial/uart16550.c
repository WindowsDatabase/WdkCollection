...

{
    ...
    //
    // Detect if the serial port exists.
    //

    if (MemoryMapped == FALSE) {
        PortExists = Uart16550DoesPortExist(Port);
        if (PortExists == FALSE) {
            return FALSE;
        }
    }
...

{
    ...
    //
    // Read the Line Status Register to determine if there is any pending data.
    //
...
Routine Description:
    ...
    This routine detects if the port object's address field contains the base
    of a UART's register set. It does this by attempting to place the port into
    diagnostic (loopback) mode. If that is successful, it will set a bit in the
    modem control register. If the UART exists then a corresponding bit should
    show up in the modem status register.

    N.B. This routine must be called before the device is enabled for
         interrupts, this includes setting the output2 bit in the modem control
         register.
...
Return Value:
    ...
    TRUE if the port object's address field contains the base of a UART's
    register set, FALSE otherwise.
...

{
    ...
    //
    // Set the port into diagnostic mode. Perform this operation twice to
    // ensure all the lower bits are clear.
    //

    Port->Write(Port, COM_MCR, SERIAL_MCR_LOOP);
    Port->Write(Port, COM_MCR, SERIAL_MCR_LOOP);
    ...
    Msr = Port->Read(Port, COM_MSR);
    if (CHECK_FLAG(Msr, SERIAL_MSR_CTS) ||
        CHECK_FLAG(Msr, SERIAL_MSR_DSR) ||
        CHECK_FLAG(Msr, SERIAL_MSR_RI) ||
        CHECK_FLAG(Msr, SERIAL_MSR_DCD)) {
        ...
        Port->Write(Port, COM_MCR, (SERIAL_MCR_OUT1 | SERIAL_MCR_LOOP));
        Msr = Port->Read(Port, COM_MSR);
        if (CHECK_FLAG(Msr, SERIAL_MSR_RI)) {
            Success = TRUE;
        } else {
            Success = FALSE;
        }
    ...
    //
    // Restore the saved modem control register.
    //

    Port->Write(Port, COM_MCR, Mcr);
    return Success;
}
