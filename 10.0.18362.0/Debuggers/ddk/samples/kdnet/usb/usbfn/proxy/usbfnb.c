...
void EnqueueTransfer(_Out_ PTransferQueue Queue, _In_opt_ PUCHAR Packet, _In_opt_ ULONG Length)
{
    ...
    if (Packet != NULL) {
        ...
        NT_ASSERT(Length <= sizeof(Queue->Packet[Queue->Tail].Buffer));

        Queue->Packet[Queue->Tail].Length = Length;
        RtlCopyMemory(&Queue->Packet[Queue->Tail].Buffer, Packet, Length);
    }
...
void DequeueTransfer(_Inout_ PTransferQueue Queue, _Out_opt_ PUCHAR Packet, _Out_opt_ PULONG Length)
{

    NT_ASSERT(Queue != NULL);

    if ((Packet != NULL) && (Length != NULL)) {
        *Length = Queue->Packet[Queue->Head].Length;
        RtlCopyMemory(Packet, &Queue->Packet[Queue->Head].Buffer, *Length);
    }
    ...
    )
{
    ...
    if (NT_SUCCESS(Status)) {
        *Handle = Index | TRANSMIT_HANDLE;
        EnqueueTransfer(&TxTransfer, NULL, 0);
    }
    ...
    )
{
    ...
    DequeueTransfer(&TxTransfer, NULL, NULL);
    return Status;
}
    ...
    )
{
    ...
    if (NT_SUCCESS(Status)) {
        ...
        *Packet = &RxTransfer.Packet[Index].Buffer[0];
        EnqueueTransfer(&RxTransfer, NULL, 0);
    }
    ...
    )
{
    ...
    DequeueTransfer(&RxTransfer, NULL, NULL);
